...
def fitnesss(chorom,population):
    fitness_value=0
    rf=0.9
    #ezafi_list=[]
    copychorom=chorom[:]
    ...
    for choromosome in population:
          ...
          winB=0
          for i in range(4):   
              if ch[i]>choromosome[i]:      
                  ezafi=(ch[i]-choromosome[i])-1
                  #chorom[i]=choromosome[i]+1
                  add=int(np.trunc(rf*ezafi))
                  #chorom[i+1]+=add
                  if (i+1)<4:
                      ch[i+1]+=add
                      ch[i]-=add
...
def elitism_select(population):
    ...
    for i in population:
        fitness_list.append(fitnesss(i,population))
...
for i in range(1000):
    ...
    avaragef.append(s[3])
    if fitnesss(chorom,population)==50:
